...
\begin{wrapfigure}[18]{TL}{0.5\textwidth}	
	\begin{center}
	\includegraphics[width=0.5\textwidth]{Billeder/I_rlocus.PNG}
	\end{center}
...
\begin{wrapfigure}[17]{TL}{0.5\textwidth}
...
Controlleren til pan systemet designes på samme måde som for tilt-systemet. Tidskonstanten er aflæst til 0.44 og DC-gain'et er $505/12$ (se figur \ref{fig:Pan_Response}). 

\begin{equation}\label{eq:pan_tf}
\frac{\omega(s)}{V_{a}(s)}=\frac{505/12}{0.44s+1}
\end{equation}

\begin{equation}\label{eq:pan_ol_tf}
\frac{Y(s)}{R(s)}=\frac{K_{P}s+K_{I}+K_{D}s^2}{s}\cdot\frac{505}{s(0.44s+1)}
\end{equation}
