...
Denne sektion kommer omkring det forskellige moduler, som vores mikroprocessor håndterer. Udover dette konmmer vi også igennem hvilket styresystem (OS) vi har valgt og hvorfor.

\subsection{OS}
Det er vigtigt i enhver kode situation, der inkluderer flere tasks, at kunne skifte mellem dem på en fornuftig måde; Dette gøres med en scheduler. Når der skal implementeres en scheduler skal det overvejes hvorvidt det er nødvendigt at lave en designet specielt til projektet, eller om der findes en eksisterende der er i stand til at udfører opgaven tilstrækkeligt. I dette project har vi to muligheder til rådighed: FreeRTOS eller RTCS.


\subsubsection{FreeRTOS}

FreeRTOS \cite{FreeRTOSorg} er et realtid operativsystem (RTOS), som er en standard løsning som bliver brugt i en mikroprocessor. FreeRTOS giver os muligheden for at bruge preemptiv skedulering, det vil sige at CPU'en kan springe i-mellem tasks og hoppe ud af en task efter behov. Dette bliver styret ud fra hvilken prioritet task'en har, eller om der er en anden task, der får et interrupt den skal holde øje med.
\\
