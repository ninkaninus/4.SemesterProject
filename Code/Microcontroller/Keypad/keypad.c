...
extern void keypad_get_task(INT8U my_id, INT8U my_state, INT8U event, INT8U data)
{
	switch(my_state)
	{
	case KEYPAD_INIT:
		keypad_init();
		set_state(KEYPAD_READY);
		break;

	case KEYPAD_READY:
	 if( keypad_pressed())
	 {
		set_state(KEYPAD_PUSHED);
	  	put_queue( Q_KEYPAD, keypad_output(), WAIT_FOREVER );
	 }
	 else
		 wait ( 1 );
	 break;
